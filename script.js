...
function genRes(move1, move2) {
    ...
    if (moves.length <= 5) {
        if (move2 > move1) {
            (move2 - move1 <= range) ? res.push(lose) : res.push(win)
        }
        if (move2 < move1) {
            (move1 - move2 <= range) ? res.push(win) : res.push(lose)
        }
    }
    if (moves.length > 5) {
        if (move2 > move1) {
            (move2 - move1 <= range1) ? res.push(lose) : res.push(win)
        }
        if (move2 < move1) {
            (move1 - move2 <= range1) ? res.push(win) : res.push(lose)
        }
    }
}
