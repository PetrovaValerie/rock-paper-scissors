...
export function getTable(...moves) {
    ...
    moves.forEach((move1) => {
        ...
        moves.forEach((move2) => {
            ...
            if (moves.length <= 5) {
                if (moveU > moveC) {
                    (moveU - moveC <= range) ? row.push(lose) : row.push(win)
                }
                if (moveU < moveC) {
                    (moveC - moveU <= range) ? row.push(win) : row.push(lose)
                }
            }
            if (moves.length > 5) {
                if (moveU > moveC) {
                    (moveU - moveC <= range1) ? row.push(lose) : row.push(win)
                }
                if (moveU < moveC) {
                    (moveC - moveU <= range1) ? row.push(win) : row.push(lose)
                }
            }
        })
        table.push(row)
